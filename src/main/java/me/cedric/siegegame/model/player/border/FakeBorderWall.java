...
public class FakeBorderWall {
    ...
    private boolean shouldDisplayNorm(Border border, Location location) {
        ...
        boolean inOuterBox = borderBox.isColliding(location);
        boolean inInnerShrunkBox = minBox.isColliding(location);
        boolean result = inOuterBox && !inInnerShrunkBox;

        SiegeGamePlugin plugin = gamePlayer.getPlugin(); 
        if (plugin != null && gamePlayer.getBukkitPlayer() != null && location.getWorld() != null && borderBox.getWorld() != null) {
        }
