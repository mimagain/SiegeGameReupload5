...
public class TerritoryController implements Listener {
    ...
    public static void setGlobalBypass(boolean bypass) {
        GLOBAL_BYPASS = bypass;
    }

    public static boolean isGlobalBypass() {
    ...
    private void evaluateCancel(Player player, Location location, Cancellable event) {
        ...
        for (Team team : worldGame.getTeams()) {
            if (team.getSafeArea() != null && team.getSafeArea().getBoundingBox().isColliding(location)) {

                if (!gamePlayer.getBukkitPlayer().hasPermission(Permissions.CLAIMS_BYPASS.getPermission()) || GLOBAL_BYPASS) {
